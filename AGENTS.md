...
Current state: **Phases 1–9 complete** — control plane, streaming text chat, a pure-Go WebRTC audio bridge, local TTS (Supertonic), local streaming ASR (Nemotron 3.5), the **live voice pipeline** (Silero VAD + semantic VAD + speak-to-interrupt barge-in + echo/backchannel handling, a shared agent loop, and a benchmark harness) — all local inference via ONNX Runtime behind the `onnx` build tag — the **per-user security boundary** (a Docker `sbx` sandbox runner, a per-user encrypted secret vault, a Sandbox Environment policy, and the shell/file/HTTP tool-execution boundary), the **callable-agent layer**: a per-user **auth broker** (interactive browser/device login in a short-lived `sbx` auth container, or an API-key/token profile) with each provider's credential store kept as **encrypted agent-state** in the same vault boundary, plus **typed adapters** (Codex/Claude/Cursor/Pi) that run each agent headlessly inside `sbx` as an `agent.<provider>.run` tool returning a normalized `AgentRunResult` — and the **Automations layer**: the versioned `automation.v1` document, a durable server-up-only **scheduler** (resume on restart, missed-run `skip`/`run_once`, concurrency policies, clean shutdown), a deterministic-then-model **step engine** (tool/condition/for_each/parallel/llm/agent_cli/finish) bound by per-run budgets and the automation's own sandbox profile, **immutable run records + promoted artifacts**, an on-rails **builder UI** with import/export, and **LLM-assisted drafting** (explain schema → JSON → validate → retry → human review before enable). Agent runs reuse the Phase 7 lock/approval/redaction/quota/audit boundary and fail closed unless `[sandbox] network_isolated` is asserted; Pi is the local-only path and stays unavailable until Phase 11 provides the managed llama.cpp endpoint. The agent CLIs + `sbx` aren't required to build — the engine + scheduler are exhaustively unit-tested with fakes, the adapters fixture-tested, the broker fake-session tested; real-CLI/container validation (the live `gh`/agent automation runs) is deferred to an `sbx`-equipped host (Linux/macOS this phase) and Windows to Phase 12. The optional MCP facade / `mcp.call` execution is also deferred. See `README.md` for the per-phase feature breakdown and `plans/roadmap.md` for what's next.
...
internal/
  ...
  vault/             Per-user secret vault: envelope encryption (per-secret AES-256-GCM data key wrapped by the platform master key), owner-private on-disk blobs (never in the DB), run-scoped env injection + a secret redactor; also stores per-provider encrypted agent-state (agentstate.go)
  sandbox/           Docker `sbx` runner (pinned + smoke-tested CLIRunner, argv/policy builders, process-tree exec), WorkspaceManager (durable/ephemeral + janitor + quota), Environment policy, the tool Router (policy + secret injection + approval + audit), and the AgentRouter (agentrun.go: runs a callable agent inside sbx with its agent-state mounted/credential injected, normalized result) + tar agent-state archiving (archive.go). CGo-free; fake-sbx-shim tested
  agentcli/          Typed, versioned adapters for the callable agent CLIs (codex/claude/cursor/pi): pure argv builders + output parsers + capability metadata + a normalized AgentRunResult (B7-verified flags; fixture-tested, no real CLI needed)
  agentauth/         Per-user agent auth broker: interactive login sessions in an `sbx` auth container (CLI factory + host pipes), sanitized output streaming with URL/device-code/paste-prompt detection, API-key/token profiles, logout. Pure detect/sanitize + fake-session tested
  automation/        Phase 9 automation.v1: the document types + structural validator + enable-eligibility check, the ${item}/step.field selector+template engine (bounded), interval/cron+timezone scheduling, and the deterministic-then-model step engine (tool/condition/for_each/parallel/llm/agent_cli/finish) that threads budgets/concurrency into an immutable run record. Pure stdlib; exhaustively unit-tested (no sbx/CLI/model)
  autorun/           Phase 9 automation runtime: the durable server-up-only scheduler (resume + missed-run skip/run_once + concurrency policies + clean shutdown), the side-effecting Executor (deterministic tools argv-first via the sbx Runner; agent_cli via the Phase 8 AgentRouter; llm via the provider), the per-user CRUD/run/artifact service, and LLM-assisted drafting. Fake-Runner/fake-store tested; real sbx/CLI validation deferred to an sbx host
