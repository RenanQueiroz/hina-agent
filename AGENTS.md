...
Current state: **Phases 1–8 complete** — control plane, streaming text chat, a pure-Go WebRTC audio bridge, local TTS (Supertonic), local streaming ASR (Nemotron 3.5), the **live voice pipeline** (Silero VAD + semantic VAD + speak-to-interrupt barge-in + echo/backchannel handling, a shared agent loop, and a benchmark harness) — all local inference via ONNX Runtime behind the `onnx` build tag — the **per-user security boundary** (a Docker `sbx` sandbox runner, a per-user encrypted secret vault, a Sandbox Environment policy, and the shell/file/HTTP tool-execution boundary), and the **callable-agent layer**: a per-user **auth broker** (interactive browser/device login in a short-lived `sbx` auth container, or an API-key/token profile) with each provider's credential store kept as **encrypted agent-state** in the same vault boundary, plus **typed adapters** (Codex/Claude/Cursor/Pi) that run each agent headlessly inside `sbx` as an `agent.<provider>.run` tool returning a normalized `AgentRunResult`. Agent runs reuse the Phase 7 lock/approval/redaction/quota/audit boundary and fail closed unless `[sandbox] network_isolated` is asserted; Pi is the local-only path and stays unavailable until Phase 11 provides the managed llama.cpp endpoint. The agent CLIs (like `sbx`) aren't required to build — the adapters are fixture-tested and the broker is fake-session tested; real-CLI/container validation is deferred to an `sbx`-equipped host (Linux/macOS this phase) and Windows browser-auth to Phase 12. The optional MCP facade (B7 item 5) is also deferred. See `README.md` for the per-phase feature breakdown and `plans/roadmap.md` for what's next.
