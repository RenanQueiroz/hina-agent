...
> **Status: Phase 3 (WebRTC audio loopback).** Phases 1–3 are complete: the cross-platform control plane, a React/Vite web client with streaming text chat, and a **pure-Go WebRTC media bridge** (Pion) that carries browser mic audio to the server and audio back to the browser — loopback or a test tone — with typed datachannel events, a playback cursor for barge-in, and live latency/loss/jitter metrics. No speech models yet; that's Phases 4–6. The default LLM provider is a credential-free **mock**, so it runs with no setup; point `[llm]` at OpenAI or a local llama.cpp server for a real model. See [`plans/roadmap.md`](plans/roadmap.md).
