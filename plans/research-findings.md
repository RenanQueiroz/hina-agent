...
**Decision: `yalue/onnxruntime_go`, pin `v1.31.0`, paired with bundled ORT `1.31.x` shared libs.** CGo, loads the ORT lib via `SetSharedLibraryPath(...)` (bring-your-own DLL/.so/.dylib — avoids MSVC/MinGW linkage). The binding tag must match the ORT lib version exactly. This is the single inference backbone for VAD + ASR + TTS, and it's the binding Supertonic's official Go example and the Silero/Nemotron references already use. No pure-Go inference path exists, so CGo is accepted **only** at this boundary (build-tagged), keeping the control plane CGo-free.
...
- Refs: github.com/yalue/onnxruntime_go (v1.31.0, 2026-06-04).
...
- **C6. `nlpodyssey/openai-agents-go` adoption — DEFERRED to Phase 6** (after the minimal custom loop and the event/session model are stable), per B9.
